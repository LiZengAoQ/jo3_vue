    ...
    <div class="app-container">
        ...
        <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="100px">
                ...
                <el-form-item label="执业证编码" prop="practicingCertificateNo">
                    <el-input
                        v-model="form.practicingCertificateNo"
                        placeholder="请输入执业证编码"/>
                </el-form-item>
                <el-form-item label="资格证编码" prop="qualificationCertificateNo">
                    <el-input
        ...
        >
            <div v-if="form2">
                <div class="one" style="border-bottom: 1px solid #e7eaec">
                    <p>停诊公告</p>
                    <p>
                        停诊开始时间： {{ parseTime(form2.noticeStartDate) }} 停诊结束时间：
                        {{ parseTime(form2.noticeEndDate) }}
                    </p>
                    <p>{{ form2.content }}</p>
                </div>
            </div>
            ...
            >
                <el-form-item label="医生名称" prop="docName">
                    <el-input
                        ...
                        clearable
                        size="small"
                        @keyup.enter.native="guahaoUpdate1"
                    />
                </el-form-item>
                <el-form-item>
                    <el-button
        ...
        <el-dialog :visible.sync="open6" width="400px">
            <h5>确定要提交嘛</h5>
            <div slot="footer" class="dialog-footer">
        ...
        <!-- <el-dialog  :visible.sync="open7" width="400px" >
              <h5>确定解绑嘛</h5>
              <div slot="footer" class="dialog-footer">
            <el-button type="primary" @click="submitForm1">确 定</el-button>
      ...
      <el-dialog  :visible.sync="open8" width="400px" >
              <h5>只允许提交一次</h5>
              <div slot="footer" class="dialog-footer">
            <el-button type="primary" @click="submitForm1">确 定</el-button>
    ...
    methods: {
        ...
        handleSelectionChange1(selection) {
            if(selection.length > 1  ){
                ...
                if (this.hayid == selection[0]["bookDocId"]) {
                    var that = this;
                    this.form4.map((item) => {
                        if (item.bookDocId == that.hayid) {
                            that.$nextTick(() => {
                                return that.$refs.multipleTable.toggleRowSelection(item, true);
                            });
                            this.hayid=null
                            this.abc =1;
                        }
                    ...
                    bingding({
                        id: this.rowid,
                        bookDocId: selection[0]["bookDocId"],
                    }).then((response) => {
                        this.msgSuccess("已绑定");
                    });
                }
                ...
                if (this.hayid == null && selection.length == 0 && this.abc ==2) {
                    bingding({
                        id: this.rowid,
                        bookDocId: null,
                    }).then((response) => {
                ...
                // this.bookDocId = selection[selection.length-1]['bookDocId']
                // this.open6=true
            }
            this.getList();
        ...
        submitForm1() {
            this.open6 = false;
            bingding({ id: this.rowid, bookDocId: this.bookDocId }).then(
                (response) => {
        ...
        guahaoUpdate(row) {
            ...
            bangdingInfo(this.queryParams).then((response) => {
                ...
                if(this.hayid ==null){ return this.abc = 1;}
                var that = this;
                if (this.hayid != null && this.hayid != "" && this.hayid != undefined) {
                    var that = this;
...
.el-table__header thead .el-table-column--selection .cell {
    display: none;
}
// 上传并修改
.image {
