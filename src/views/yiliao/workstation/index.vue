        ...
        <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="80px">
                <el-row>
                    ...
                    <el-col :span="12">
                        <el-form-item v-if="form.type != 'B'" label="是否外链">
                    ...
                    <el-col :span="12">
                        <el-form-item v-if="form.type != 'B'" label="路由地址" prop="url">
                    ...
                    <el-col :span="12">
                        <el-form-item v-if="form.type != 'D'" label="权限标识">
                            <el-input v-model="form.code" placeholder="请权限标识" maxlength="50"/>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item v-if="form.type != 'B'" label="显示状态">
                            <el-radio-group v-model="form.visible">
                                <el-radio
                                    v-for="dictionary in visibleOptions"
                                    :key="dictionary.name"
                    ...
                    <el-col :span="12">
                        <el-form-item v-if="form.type != 'B'" label="菜单状态">
                    ...
                    <el-col :span="12">
                        <el-form-item v-if="form.type == 'M'" label="是否缓存">
...
export default {
    ...
    created() {
        this.getList()
        getDictionarys('sys_show_hide').then(response => {
    ...
    methods: {
        ...
        getList() {
            this.loading = true
            listGzzPermission(this.queryParams).then(response => {
                this.permissionList = this.handleTree(response.data, 'id')
                this.loading = false
            })
        },
        /** 转换菜单数据结构 */
        ...
        /** 查询菜单下拉树结构 */
        getTreeselect() {
            listGzzPermission().then(response => {
                this.permissionOptions = []
                const menu = { id: 0, name: '主类目', children: [] }
                menu.children = this.handleTree(response.data, 'id')
                this.permissionOptions.push(menu)
            })
        },
        // 显示状态字典翻译
        ...
        handleUpdate(row) {
            this.reset()
            this.getTreeselect()
            getGzzPermission(row.id).then(response => {
            ...
            this.$refs['form'].validate(valid => {
                if (valid) {
                    if (this.form.id != undefined) {
                        updateGzzPermission(this.form).then(response => {
                            this.msgSuccess('修改成功')
                            this.open = false
                            this.getList()
                        })
                    } else {
                        addGzzPermission(this.form).then(response => {
        ...
        handleDelete(row) {
            this.$confirm('是否确认删除名称为"' + row.name + '"的数据项?', '警告', {
                ...
                return delGzzPermission(row.id)
            }).then(() => {
                this.getList()
